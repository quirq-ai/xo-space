...
The Work tab is where a person sees everything that needs them across the Space: work items assigned to them or to nobody, blocked todos, issues for them, mail and mentions of a listed kind, failing sources, and what finished. Those arrive by themselves from the logs. This file is for the other case: an agent that has something a person should look at (a question, a finding, a request, a result) posts it here instead of burying it in a transcript. `POST /api/inbox` still works and means the same thing.
...
When a connection folder exists (`~/.quirq/work/inbox/<connection>/`), the
items the person listed arrive as folders and may start a session that runs
the `inbox-item` skill. If you are that session, the prompt already tells you
everything; end with the outcome block. If you are an ordinary project session,
`GET /api/work/inbox/items?status=new` lists what waits, and
`POST /api/work/inbox/items/{connection}/{id}/start` runs it (202, or 409 when
it already ran or the connection's sessions are off).
