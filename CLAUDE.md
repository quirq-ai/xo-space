...
- Routes that serve the xo-cowork frontend live under `api/`, where the folder is the URL (`api/files/routes.py` serves `/api/files/*`; `routers/autoroutes.py` mounts the tree from `config/autoroutes.json`). Shared helpers live under `services/cowork_agent/`.
- Placement rule of thumb: only functions specific to running an agent go under `services/cowork_agent/`. Anything a person uses as much as the agent does (the Inbox, connections polling, the swarm client) is a property of the Space and is a top-level package under `services/` (`services/inbox/`, `services/connections/`, `services/swarm_api/`). Judge by the consumer, not the dependency: connections depend on Composio, which the agent also uses, and still belong at the top level. What those packages share is Space-level too: `services/storage/` (the file primitives, moved out of the agent tree; the old `cowork_agent/visualizer/{flock,atomic_write,reader}` and `cowork_agent/local_state` import paths still resolve to the same modules), `services/timestamps.py`, `services/errors.py` (`ServiceError`, the base of every typed service failure), `services/periodic.py` (`run_forever`, the shared poll loop) and `routers/errors.py` (`http_error`, `ForbidExtra`). `services/connections` never imports the inbox; the inbox registers a new-events listener with it. See DEVELOPING.md section 7.
