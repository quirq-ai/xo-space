...
**First run.** Space opens on **Projects → Overview** (`#/projects/overview`). Projects contains **Overview | Files | Timeline | Manage**, with **List | Graph | Tree** views inside Files. Choose **Files → List** (`#/projects/files/list`) to browse the folders in your workspace, or open **Manage → Add project** to clone a Git repository. Before the first chat, check Setup → Intelligence layer for the agent connection and activity collection settings. Full walkthrough: [INSTALLATION.md](INSTALLATION.md).

**Finding your projects.** Search by name, folder ID or description, filter to **Live** sessions or **Pinned** projects, and sort by recent activity, name, indexed files or creation date. Pins stay in this browser. Open a Files List row to browse files; folder position and filters survive navigation, and **Refresh files** rereads the current folder. File counts describe the visible file index, with `+` when scanning was incomplete; a project missing from the index shows **Not indexed**. **Graph root**, available throughout Projects, opens the graph at the chosen node.

**Managing projects.** **Projects → Manage** (`#/projects/manage`) contains cloning, inline sharing and guarded local removal. Expand a project card to view its metadata and GitHub Issues, copy recorded values, or open its activity. Sharing is also available directly on Files List rows: enter the destination Space ID without leaving the page. Individual access must be revoked before a shared project can be deleted locally. The old `#/setup/projects` address opens Manage.
