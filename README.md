...
**First run.** Space opens on **Projects → List** (`#/projects/list`) to browse every folder in the directory you installed in. Projects, Agents, Inbox and Setup share **List | Graph | Tree** controls: List contains their existing forms and actions, while Graph and Tree show the relationships between loaded records. Returning to List keeps your last page and drafts. Projects also has Timeline and Sharing pages. An empty List shows *No projects yet*. Choose **Add project** to clone a Git repository in Setup → Projects, or **Getting started** for the Wiki walkthrough. You can also create a folder directly in your workspace or ask your agent to create an xo-project. Before the first chat, check Setup → Intelligence layer: the agent CLI is on PATH (`npm install -g @anthropic-ai/claude-code`) and a credential is saved. Full walkthrough: [INSTALLATION.md](INSTALLATION.md).

**Finding your projects.** Search by name, folder ID or description, filter to **Live** sessions or **Pinned** projects, and sort by recent activity, name, indexed files or creation date. Pins stay in this browser. Open a row for **Files**, **Activity** (todos, sessions and events), or **Issues**; your folder and filters stay in place while you browse. **Refresh details** updates the selected tab. File counts describe the visible file index, with `+` when scanning was incomplete; a project missing from the index shows **Not indexed**.

**Setup and management.** Setup groups Workspace, Intelligence layer, and Projects into three steps. Intelligence layer brings agent connection and activity collection together; Projects handles repository cloning and guarded local removal. Workspace also shows the Space ID, workspace owner, XO account and verified GitHub identity. Open Connectors for app connections, permissions and polling; Secrets for environment keys; Commands for saved commands and run results; or Server for updates, restart and technical details. Each section has a direct URL: `#/setup/workspace`, `#/setup/intelligence`, `#/setup/projects`, `#/setup/connectors`, `#/setup/secrets`, `#/setup/commands` and `#/setup/server`. Opening Setup starts at Workspace. Existing `#/setup`, `#/connectors` and `#/secrets` links still work and resolve to their corresponding section URLs.
...
To see what your install decided: open **Setup → Intelligence layer → Usage reporting** (`/space/#/setup/intelligence`), the server's own decisions are in `<state root>/quirq.log` (`grep usage_sync ~/.quirq/quirq.log` on the default install), and every external command Quirq runs is recorded beside it in `<state root>/commands.log`. The installer prints the log pointers on every run.
