...
OAuth callback (api/connectors/vercel/routes.py). This folder must be
mounted BEFORE ``api/connectors/vercel`` (``MOUNT_ORDER`` in this package's
``__init__.py`` pins it; the loader in routers/autoroutes.py sorts
subpackages by it, then by name): the dispatcher below
...
from api.connectors.vercel.routes import vercel_oauth_callback

log = logging.getLogger(__name__)
router = APIRouter()
# /callback is a fixed redirect target, not an /api/connectors/magicpath URL.
...
@absolute_router.get("/callback", include_in_schema=False)
