...
| `POST /api/inbox` | alias of the above, kept | |
...
`GET`, `PATCH` and `DELETE /api/inbox` served the old page and go with it. The
skill reference `inbox-http-api.md` becomes `work-http-api.md` and keeps the
one paragraph an agent needs: post here when a person should look.
