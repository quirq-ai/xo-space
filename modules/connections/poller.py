...
def tick_seconds() -> float:
    """``tasks.poller.tick_s`` when a person set it in Setup (the overrides
    file), else the env variable, else the manifest's default; never below
    :data:`MIN_TICK_S`."""
    try:
        from services import modules as registry
        chosen = registry.overrides().get("connections", {}).get("tasks", {}).get("poller", {})
        configured = chosen.get("tick_s") if isinstance(chosen, dict) else None
    except Exception:  # noqa: BLE001 - the registry is optional here (tests, the CLI)
        configured = None
    if isinstance(configured, (int, float)) and not isinstance(configured, bool):
        return max(MIN_TICK_S, float(configured))
...
async def poll_once() -> dict:
    ...
    for toolkit in toolkits:
        ...
        if outcome.get("new_events"):
            # The same signal ``poll_now`` raises: listeners (the Work's item
            # maker, the inbox ingest) react to the tick as well as to a click.
            from services import signals
            await signals.notify("connections.new_events", toolkit=toolkit)
    return summary
