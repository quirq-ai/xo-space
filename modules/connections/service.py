...
SIGNAL_MODULE = "connections"
from .poller import poller_enabled  # re-exported: the router reports it next to signed_in
from .store import ConnectionsError  # re-exported: tests and callers name it as service.ConnectionsError
...
def _check_known(toolkit) -> str:
    ...
    any path is built (the store repeats the regex check on its own). A
    value that is not even id-shaped is not echoed back."""
    if not isinstance(toolkit, str) or store.TOOLKIT_RE.fullmatch(toolkit) is None:
        raise ConnectionsError("unknown_toolkit", "Unknown toolkit.", 404)
...
# The signal ``connections.new_events`` (declared in events.SIGNALS) fires
# after any poll that collected something, from the loop's tick as well as
# from ``poll_now``. Listeners live in other modules' ``listeners.py``;
# :func:`register_new_events_listener` is kept for callers outside the
# module contract and registers through ``services.signals``.


def register_new_events_listener(fn) -> None:
    """Register ``async fn(toolkit)`` to run after a poll that collected
    something. Registering the same callable again is a no-op."""
    signals.on(f"{SIGNAL_MODULE}.new_events", fn, module="runtime")


async def _notify_new_events(toolkit: str) -> None:
    """Every enabled listener, in order; a failing one is logged and skipped."""
    await signals.notify(f"{SIGNAL_MODULE}.new_events", toolkit=toolkit)
