...
#: The names rotation produces (services/storage/eventlog.py); only these are ever pruned.
_ROTATION_RE = re.compile(r"events\.\d{8}T\d{6}Z(\.\d+)?\.jsonl")
...
def events_log(toolkit: str) -> EventLog:
    """The toolkit's ``events.jsonl`` as an :class:`EventLog` (rotation at
    :data:`_ROTATE_BYTES`, :data:`_MAX_ROTATIONS_KEEP` segments kept)."""
    return EventLog(_events_path(toolkit), rotate_bytes=_ROTATE_BYTES, keep=_MAX_ROTATIONS_KEEP)
...
def append_events(toolkit: str, events: list[dict]) -> int:
    ...
    if not (path.parent / "config.json").is_file():
        ...
        return 0
    return events_log(toolkit).append(ordered)
...
def read_events(toolkit: str, limit: int = 50, types=None) -> list[dict]:
    ...
    file order reversed)."""
    _check_toolkit(toolkit)
    return events_log(toolkit).tail(limit=max(1, int(limit)), types=types)
