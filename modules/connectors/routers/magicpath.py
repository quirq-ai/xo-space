...
``vercel_router`` (see modules/connectors/routes.py): the dispatcher below
...
async def probe_status() -> dict:
    ...
    so a valid session with the network down reports false. The status
    route answers this, and ``modules.connectors.service.magicpath_status``
    reports it: MagicPath's CLI wrapper lives in this module until it has a
    package of its own.
    """
    cli = _cli_binary()
