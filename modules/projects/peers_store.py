...
def _corrupt(path: Path, reason: str) -> PeersStoreError:
    return refused_document(
        PeersStoreError, path, reason,
        name="peers.json", document="peers", loss="every collaborator it lists",
    )
...
def _read_document(path: Path) -> tuple[Optional[str], list[dict]]:
    ...
    if state == "schema":
        raise unsupported_schema(
            PeersStoreError, path, value, PEERS_SCHEMA, name="peers.json",
        )
    parsed = value
...
def update_peer(
    ...
    *,
    ...
    with locked(peers_path):
        ...
        index = _index_of(peers, user_id)
        if index is None:
            raise PeersStoreError("peer_not_found", "Peer not found.", 404)
        record = peers[index]
