...
class TodosStoreError(StoreError):
    """Base for all store failures: ``(code, message)`` plus the status the
    raise site chose (see :class:`StoreError`); the app handler serves it."""
    ...
    if state == "schema":
        if isinstance(value, int) and not isinstance(value, bool) and value < TODOS_SCHEMA:
            ...
            raise unsupported_schema(
                TodosStoreError, todos_path, value, TODOS_SCHEMA, name="todos.json",
            )
    else:
        current = value
    raw = current.get("sessions")
...
def _corrupt(path: Path, reason: str) -> TodosStoreError:
    return refused_document(
        TodosStoreError, path, reason,
        name="todos.json", document="todos", loss="every todo it holds",
    )
    ...
    *,
    ...
    with locked(todos_path):
        ...
        if _find(sessions, todo_id) is not None:
            raise TodosStoreError(
                "scope_unavailable", "todos.json write failed.", 500,
                log=f"todo id collision ({todo_id}); retry the call.",
            )
    ...
    *,
    ...
    with locked(todos_path):
        ...
        if found is None or is_deleted(found[2]):
            raise TodosStoreError("todo_not_found", "Todo not found.", 404)
