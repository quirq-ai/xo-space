...
from services.storage.document import CORRUPT_DOCUMENT_MESSAGE
...
def _corrupt(path: Path, reason: str) -> WorkitemClaimsError:
    return WorkitemClaimsError(
        "corrupt_document",
        CORRUPT_DOCUMENT_MESSAGE.format(name="claims.json"),
        409,
        log=(
...
def read_claims(path: Path) -> dict[str, dict]:
    ...
    if state == "schema":
        raise unsupported_schema(
            WorkitemClaimsError, path, value, CLAIMS_SCHEMA, name="claims.json",
        )
    parsed = value
...
def _emit(path: Path, events: list[Event], *, project_id: Optional[str] = None) -> None:
    """Append claim events to the project's timeline (the runtime home the
    claims file sits in), then raise ``projects.workitem_changed`` for each
    claim and release. **Never raises.**"""
    if not events:
        return
    timeline.apply_quiet(Path(path).parent.parent, events, project_id=project_id)
    for event in events:
        if isinstance(event, WorkitemEvent):
            module_events.workitem_changed(project_id or Path(path).parent.parent.name,
                                           event.workitem_id, event.action)
