...
from services.cowork_agent.visualizer.atomic_write import read_stamped_document
...
from . import events as module_events
...
def emit_workitem_events(project_id: str, events: Iterable[Event]) -> None:
    """Append workitem lifecycle events to a project's timeline, then raise
    ``projects.workitem_changed`` for the transitions other modules listen
    for (``events.WORKITEM_CHANGE_ACTIONS``)."""
    events = list(events)
    try:
        ...
        logger.warning(
            ...
            project_id, len(events), exc_info=True,
        )
        root = None
    if root is not None:
        timeline.apply_quiet(root, events, project_id=project_id)
    for event in events:
        if isinstance(event, WorkitemEvent):
            module_events.workitem_changed(project_id, event.workitem_id, event.action)
...
def _corrupt(path: Path, reason: str) -> WorkitemsStoreError:
    return refused_document(
        WorkitemsStoreError, path, reason,
        name="workitems.json", document="workitems", loss="every workitem it holds",
    )
...
def _read_document(path: Path) -> dict:
    ...
    if state == "schema":
        raise unsupported_schema(
            WorkitemsStoreError, path, value, WORKITEMS_SCHEMA, name="workitems.json",
        )
    parsed = value
    ...
    *,
    ...
    with locked(workitems_path):
        ...
        if workitem_id in items:
            ...
            raise WorkitemsStoreError(
                "scope_unavailable", "workitems.json write failed.", 500,
                log=f"workitem id collision ({workitem_id}); retry the call.",
            )
    ...
    *,
    ...
    with locked(workitems_path):
        ...
        if not isinstance(record, dict) or is_deleted(record):
            raise WorkitemsStoreError("workitem_not_found", "Workitem not found.", 404)
    ...
    *,
    ...
    with locked(workitems_path):
        ...
        if workitem_id is None:
            ...
            if not isinstance(record, dict) or is_deleted(record):
                raise WorkitemsStoreError(
                    "workitem_not_found", "Workitem not found.", 404,
                )
