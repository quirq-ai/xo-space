...
Moved from ``services/cowork_agent/session_transcript.py``, which still
resolves to this module. ``load_all_sessions``, ``find_session_backend`` and
``try_load_capability`` stay module-level names on purpose: the tests patch
them here.
"""
...
class SessionNotFound(ServiceError, LookupError):
    """No session has this id. A :class:`ServiceError` without a code, so
    the app's handler answers the bare ``{"detail": "Session not found"}``
    the session routes have always given (404); still a ``LookupError``
    for the callers that caught it as one."""

    def __init__(self, session_id: str) -> None:
        super().__init__(None, "Session not found", 404)
