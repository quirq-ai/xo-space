...
The index is partitioned: one shard per row under
``projects/<pid>/sessions/sessionslist.d/`` (``store.FILES`` names it),
merged on read with the pre-T19 whole-file indexes. Adapters write their
rows through :func:`write_session_row`; this module adds
:func:`find_session_row` and :func:`update_session_row` so the sessions
service can stamp ``purpose`` on a row without knowing which adapter wrote
it. :func:`load_all_sessions` is the listing every route builds on; it
routes each row's enrichment through the row's own backend's ``sessions``
capability and names no backend itself.

Moved from ``services/cowork_agent/engine/sessions_io.py``, which still
...
def find_session_row(session_id: str) -> tuple[str, str, dict] | None:
    """``(project_id, composite_key, row)`` of the index row whose
    ``sessionId`` is ``session_id``, or ``None`` when no project holds one.
    The row is a copy."""
    if not session_id:
        return None
    for project_id, _project_dir, index in iter_project_session_indexes():
        for key, meta in index.items():
            if meta.get("sessionId") == session_id:
                return project_id, key, dict(meta)
    return None


def update_session_row(session_id: str, **fields) -> bool:
    """Merge ``fields`` into the row whose ``sessionId`` is ``session_id``
    and rewrite its shard. Returns whether the row exists and now carries
    the fields; a row that already does is left untouched."""
    found = find_session_row(session_id)
    if found is None:
        return False
    project_id, key, row = found
    if all(row.get(name) == value for name, value in fields.items()):
        return True
    row.update(fields)
    return write_session_row(project_id, key, row)
    ...
    def _ingest_project_index(index_data: dict, agent_name: str, project_dir: Path) -> None:
        for key, meta in index_data.items():
            ...
            sessions.append({
                ...
                "time_archived": None,
                # The index row's own tags, beside the SessionResponse shape
                # the chat surface reads: which backend wrote the row and
                # why the session exists (modules/sessions stamps purpose).
