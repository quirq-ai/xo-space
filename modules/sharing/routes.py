...
Declarative over ``modules.sharing.service``; its typed
errors carry their own status and reach the wire through the app's service
error handler (``routers/errors.py``). No os/pathlib in this module (BFF
rule P2). The browser never talks to swarm and never sees the token.
"""
...
def _workspace_id_or_422(body: ShareBody, what: str) -> str:
    if not is_valid_workspace_id(body.workspace_id):
        raise ServiceError("missing_workspace_id", f"Enter the {what} workspace id.", 422)
...
async def share_project(project_id: str, body: ShareBody) -> dict:
    return await service.share(project_id, _workspace_id_or_422(body, "recipient's"))


@router.post("/api/xo-projects/{project_id}/revoke")
async def revoke_project(project_id: str, body: ShareBody) -> dict:
    return await service.revoke(project_id, _workspace_id_or_422(body, "revoked"))
