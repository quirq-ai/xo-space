"""Pure transforms from the watcher's ``stats.json`` blocks to the usage wire
payloads.

The project-scope and the workspace-scope usage routes read different
``stats.json`` files (``~/.quirq/projects/<pid>/stats.json`` and
``~/.quirq/cache/stats.json``) but shape them into the same models
(``models.py``). Everything here is a pure function of its arguments (no
scope, no I/O), so the two tiers differ only in which scope they read and
in the genuinely scope-specific assembly (per-session summaries, the
sessionslist union), never in how a stats block becomes tokens, tools,
models or latency. ``routers/cowork_agent/bff/_visualizer_presenter.py``
...
def bad_query(message: str) -> ServiceError:
    """The 400 a malformed query earns: ``{"code": "invalid_query",
    "message"}`` on the wire, as the usage routes have always answered."""
    return ServiceError("invalid_query", message, 400)
