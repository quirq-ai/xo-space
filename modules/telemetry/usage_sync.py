...
Runs as the telemetry module's ``usage_sync`` task (``modules/telemetry/tasks.py``),
supervised like every module's loops. Was ``services/usage_sync.py``; that
path is an alias of this module. The
...
from .usage_loader import load_usage_module
...
# its own independent sync state. Switching AGENT_NAME (say, from one agent
# to another) lands on a fresh path with no watermark, which correctly
# triggers a full backfill for the new agent rather than reusing the
# previous agent's truncation point.
# modules/telemetry/usage_sync.py: the checkout is three levels up.
