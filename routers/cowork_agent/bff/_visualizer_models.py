"""Pydantic response models for the project routes (todos, workitems, peers,
presence, the timeline, the GitHub mirror). The usage models moved to
``modules/telemetry/models.py`` and are re-exported below."""
...
# ── the usage models (moved to modules/telemetry/models.py) ──────────────────
# Re-exported under their old names for one release: the usage routes and
# their transforms live in ``modules/telemetry`` now.

from modules.telemetry.models import (  # noqa: E402, F401
    AnalyticsStats,
    CostAndTokensEntry,
    DailyBreakdownEntry,
    DailyCostEntry,
...
class TimelineEvent(BaseModel):
    """The event envelope, typed; every other key passes through as the
    module that emitted it wrote it (``extra="allow"``: the types are
    open, see ``modules/timeline``)."""
    ...
    # state root had folders; absent on older lines and on Space-level lines.
    pid: Optional[str] = None
    # The project's folder name: stamped on write when the writer knew it,
    # resolved from the pid on the merged Space read.
    project_id: Optional[str] = None
    session_id: Optional[str] = None
    runtime: Optional[str] = None
