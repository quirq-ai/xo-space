...
# Include the Space-level session index even though execution stays on the host.
...
def enrich_project_session(meta: dict, key: str, default_agent: str):
    path = resolve_native_file(meta, str(meta.get("sessionId") or ""))
    records = _read_transcript_lines(path) if path else []
    return _first_timestamp(records), _title_from_records(records), default_agent
...
def resolve_native_file(meta: dict, session_id: str) -> Path | None:
    ...
    candidate = native or lookup_seat(session_id) or session_id
...
def list_native_sessions() -> list[dict]:
    ...
    if not root.is_dir():
        return []
    rows: list[dict] = []
    indexed_seats = {row.get("nativeSessionId") for row in indexed_sessions().values()}
    try:
        names = sorted(p.name for p in root.iterdir() if p.is_dir())
    except OSError:
        return []
    for agent_id in names:
        if agent_id in indexed_seats:
            continue
...
def owns_session(session_id: str) -> bool:
    ...
    if not session_id or not is_valid_sand_agent_id(session_id):
        return False
    try:
        return resolve_native_file({}, session_id) is not None
    except ValueError:
        return False
...
def _convert_messages(session_id: str, records: list[dict[str, Any]]) -> list[dict]:
    ...
    def attach_result(block: dict) -> None:
        call_id = block.get("tool_use_id") or block.get("id")
        for part in pending_tools:
            if call_id and part["data"]["call_id"] != call_id:
                continue
            state = part["data"]["state"]
            output = block.get("result")
            state["output"] = block.get("content") if output is None else output
            state["status"] = "error" if block.get("is_error") else "completed"
            pending_tools.remove(part)
            break
