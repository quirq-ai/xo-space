"""Read-only, bounded access to Space data for authenticated MCP clients."""
...
def _safe_path(root: Path, *parts: str) -> Path:
    ...
    for part in parts:
        path = path / part
        if path.is_symlink():
            raise ValueError("Symbolic links are not available through the Space MCP server.")
...
def _check_json_size(path: Path) -> None:
    if path.is_file() and path.stat().st_size > _JSON_BYTES:
        raise ValueError("This Space data file is too large to read through MCP.")


def _list_projects(limit: int, offset: int) -> dict:
    root = project_layout.xo_projects_root(create=False)
    projects = []
...
def _read_project_document(project_id: str, document: ProjectDocument) -> dict:
...
def _list_todos(project_id: str, limit: int) -> dict:
    project = _project_path(project_id)
...
def _list_inbox(status: InboxStatus, limit: int) -> dict:
    path = inbox_store.inbox_path()
    path = _safe_path(path.parent.parent, path.parent.name, path.name)
    _check_json_size(path)
    # Supplying the path bypasses the store's legacy migration; reading MCP
...
async def _read(operation, *args) -> dict:
    try:
        ...
        raise ToolError("Space could not read that data. Check its files and permissions locally.") from None
    except ValueError as exc:
        # SDK 2 hides unexpected exceptions, including ValueError. These
        # deliberate validation failures carry safe, actionable messages.
