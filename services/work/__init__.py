...
* :mod:`items`      every Inbox row as a folder under ``work/inbox/<section>/<id>/``
                    (connections, workspace, projects, issues, agents): the
                    record, its session, its outcome and its thread.
* :mod:`runner`     one agent session per item, started by itself under the
                    section's policy, resumed by the person's replies.
