...
from . import inbox_view, items, readers, store
...
def _tracked_keys() -> frozenset[str]:
    """The source keys of the work items the connections feed made (section
    18): those events never surface as raw ``connection`` decisions too."""
    keys = set()
    for row in inbox_view.rollup_rows():
        source = row.get("source") if isinstance(row.get("source"), dict) else {}
        if source.get("kind") == "connection" and isinstance(source.get("key"), str):
            keys.add(source["key"])
...
def connection_items(doc: dict, promoted: dict) -> list[dict]:
    ...
    for entry in readers.connections(doc, limit=readers.CONNECTIONS_FETCH):
        if entry["kind"] not in kinds or entry["key"] in promoted or entry["key"] in tracked:
            continue
...
def question_items(doc: dict, promoted: dict, acked: dict) -> list[dict]:
    ...
    for post in doc["posts"]:
        if post.get("kind") not in _QUESTION_KINDS:
            continue
...
def item_items(doc: dict) -> list[dict]:
    """The Inbox work items that need a person (section 18): new ones with
    no session, done ones whose session asked, drafted or proposed, and
    failed ones. Closed items never surface."""
    out = []
    policies: dict[str, dict] = {}
    for row in inbox_view.workitem_rows():
        state = row["state"]
        if state in ("closed", "running") or row.get("fact") is None:
            continue   # only a fed work item (one with a fact) is the Inbox's to handle
        section = row["section"]
        policy = policies.setdefault(section, items.read_policy(section))
        ...
        if state == "new":
            reason = "item_new"
            origin = fact.get("toolkit") or (row.get("source") or {}).get("kind") or section
            detail = f"Arrived from {origin} ({fact.get('kind')}). " + (
                "Start a session to handle it, or archive it." if mode != "off"
                else "Sessions are off for this section; archive it or reply by hand.")
        elif state == "failed":
            reason = "item_failed"
            exit_ = session.get("exit") or {}
            detail = f"The session {exit_.get('status') or 'failed'}: {exit_.get('message') or 'no detail'}. Retry runs it again."
        elif state == "waiting":
            reason = _ITEM_OUTCOME_REASON.get(outcome.get("kind"))
            if reason is None:
                continue
            if reason == "item_question":
                detail = outcome.get("question") or outcome.get("summary") or "The session has a question."
            elif reason == "item_draft":
                detail = (outcome.get("summary") or "A reply is drafted.") + (f" Draft: {outcome['draft']}." if outcome.get("draft") else "")
            else:
                task = outcome.get("task") or {}
                detail = (outcome.get("summary") or "") + (f" Proposed: {task.get('title')}." if task.get("title") else "")
        else:
            continue
        out.append(_item(key=row["key"], reason=reason, title=row["title"], detail=detail,
                         since=row.get("updated_at") or row.get("created_at"), project_id=row["project_id"], pid=row.get("pid"),
                         source=(row.get("source") or {}).get("kind") or section,
                         ref={"section": section, "entity": row.get("entity"), "workitem_id": row["id"], "project_id": row["project_id"],
                              "kind": fact.get("kind"), "url": fact.get("url"), "link": fact.get("link"),
                              "session_id": session.get("session_id"), "native_session_id": session.get("native_session_id"),
                              "workbench": f"items/{row['id']}", "outcome": row.get("outcome"),
                              "task": outcome.get("task"), "draft": outcome.get("draft"),
                              "can_send": bool(policy["sessions"]["act"] and outcome.get("kind") == "reply_drafted"),
                              "mode": mode},
