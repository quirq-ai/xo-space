...
from . import attention, items, readers, store
...
def item_rows() -> list[dict]:
    """Inbox items a session handled or found merely worth a glance
    (section 17.3), until the person acknowledges them."""
    out = []
    for toolkit in items.list_connections():
        for item_id, s in items.read_index(toolkit)["items"].items():
            if s.get("decided") or s.get("status") != "done" or s.get("outcome") not in ("fyi", "handled"):
                continue
            outcome = items.read_outcome(toolkit, item_id) or {}
            session = items.read_session(toolkit, item_id) or {}
            acted = outcome.get("acted") or []
            out.append(_row(key=items.item_key(toolkit, item_id), kind="item", ts=s.get("updated_at") or s.get("ts"),
                            title=s.get("title") or item_id,
                            detail=(outcome.get("summary") or "") + (" · " + "; ".join(acted) if acted else ""),
                            tone="closed", actor={"runtime": session.get("runtime"), "session_id": session.get("native_session_id")} if session else None,
                            ref={"toolkit": toolkit, "item_id": item_id, "session_id": s.get("session_id"),
                                 "native_session_id": session.get("native_session_id"), "outcome": outcome.get("kind")},
                            primary=""))
    return out


def running_items() -> list[dict]:
    """Items whose session runs now (for the Live row and the header)."""
    out = []
    for toolkit in items.list_connections():
        for item_id, s in items.read_index(toolkit)["items"].items():
            if s.get("status") not in ("running", "queued"):
                continue
            session = items.read_session(toolkit, item_id) or {}
            out.append({"key": items.item_key(toolkit, item_id), "toolkit": toolkit, "item_id": item_id, "title": s.get("title"),
                        "status": s.get("status"), "session_id": s.get("session_id"), "runtime": session.get("runtime"),
                        "started_at": session.get("resumed_at") or session.get("started_at"), "project_id": items.project_id_for(toolkit)})
    out.sort(key=lambda r: parse_ts(r.get("started_at")) or _EPOCH, reverse=True)
    return out


def connections_summary() -> list[dict]:
    """One row per connection folder: the policy's mode and the counts."""
    out = []
    for toolkit in items.list_connections():
        policy = items.read_policy(toolkit) or items.normalize_policy({})
        counts = {status: 0 for status in items.STATUSES}
        waiting = 0
        for s in items.read_index(toolkit)["items"].values():
            counts[s.get("status") or "new"] = counts.get(s.get("status") or "new", 0) + 1
            if not s.get("decided") and s.get("status") in ("new", "done", "failed"):
                waiting += 1
        out.append({"toolkit": toolkit, "mode": policy["sessions"]["mode"], "act": policy["sessions"]["act"],
                    "collectors": [c for c, on in policy["items"].items() if on], "counts": counts, "waiting": waiting,
                    "project_id": items.project_id_for(toolkit)})
    return out


def completed(doc: dict, *, now: Optional[datetime] = None, rows=None) -> list[dict]:
...
def compose(doc: dict, *, me=None, now: Optional[datetime] = None) -> dict:
    """Everything the Inbox page paints, in one read."""
    ...
    groups = {"decisions": decisions(doc, me=who, rows=rows), "calendar": meetings(doc, now=now),
              "completed": completed(doc, now=now, rows=rows), "work": open_work(rows)}
    counts = {name: len(group) for name, group in groups.items()}
    try:
        running = running_items()
        connections = connections_summary()
    except Exception:  # noqa: BLE001 - a bad connection folder must not fail the page
        logger.warning("work inbox: could not read the connection folders", exc_info=True)
        running, connections = [], []
    ...
    return {"schema": store.SCHEMA, "updated_at": doc.get("updated_at"), "generated_at": iso(_now(now)),
            ...
            "badge": counts["decisions"] + counts["completed"], "running": running, "connections": connections, **groups}
