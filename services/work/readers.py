...
def legacy_posts() -> list[dict]:
    """What agents posted to the retired Inbox (``inbox/inbox.json`` items
    with no feeder key), read in place until ``POST /api/inbox`` moves here."""
    doc = read_json(inbox_dir() / "inbox.json")
    items = doc.get("items") if isinstance(doc, dict) else None
    out = []
    for it in items if isinstance(items, list) else []:
        if not isinstance(it, dict) or it.get("key") or not isinstance(it.get("id"), str) \
                or not isinstance(it.get("title"), str) or parse_ts(it.get("ts")) is None:
            continue
        out.append({"id": it["id"], "ts": it["ts"], "source": it.get("source") or "api", "kind": it.get("kind") or "note",
                    "title": it["title"], "body": it.get("body") if isinstance(it.get("body"), str) else "",
                    "project_id": it.get("project_id"), "pid": it.get("pid"), "link": it.get("link"),
                    "url": it.get("url") if store.is_url(it.get("url")) else None, "ref": None,
                    "legacy_status": it.get("status")})
    return out


def posts(doc: dict, *, limit: int, before: Optional[str] = None) -> list[dict]:
    """The agents' notes: ``history/history.json`` posts and the retired Inbox's."""
    out = [post_entry(p) for p in doc["posts"] if _keep(p.get("ts"), before)]
    seen = {e["key"] for e in out}
    for p in legacy_posts():
        if not _keep(p["ts"], before):
            continue
        entry = post_entry(p)
