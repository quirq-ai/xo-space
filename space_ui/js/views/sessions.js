...
import {icons,button,badge,card,table,sortHead,checkbox,label,toggleGroup,pagination,skeleton,alert,breadcrumb,empty,item,itemGroup,itemSeparator,spinner,switchControl,input,field,tabs,wireTabs} from '../core/shadcn.js?v=20260915-agents2';
import {areaChart,barChartHorizontal,barChartStacked,donutChart,radialChart,heatmapChart,sparkline} from '../core/chart.js?v=20260915-agents2';
...
const srcTab=new Map();          /* source id -> open card tab (usage | data) */
...
function scheduleTelemetryReload(){
  clearTimeout(telemetryRebuild);
  telemetryRebuild=setTimeout(async()=>{
    if(!SD)return;
    await load({quiet:true});
    /* the status pill now reflects the change; the "saved" notes are stale */
...
function render(){
  ...
  if(loading){
    wrap.innerHTML=pageHead+'<div role="status" aria-label="Loading agent activity" class="sess-skeleton">'
      +'<div class="sess-grid4">'+[0,1,2,3].map(()=>card({content:skeleton('height:14px;width:40%')+skeleton('height:28px;width:60%;margin-top:10px')+skeleton('height:12px;width:70%;margin-top:12px')})).join('')+'</div>'
...
function rOverview(el){
  ...
  el.innerHTML='<div class="sess-grid4">'
    +statCard('Total tokens',tok(tokens),'Across selected sources, '+winText(),{tone:'primary',action:badge(esc(winLabel()),{variant:'outline'})})
    +statCard('Cost',costState.value,esc(costState.label),{attrs:'title="'+esc(costState.title)+'"'})
    +statCard('Sessions',String(nsess),'With activity '+winText())
    +statCard('Tokens per session',nsess?tok(tokens/nsess):'0','Average over active sessions')
    +'</div>'
...
/* xo-swarm StatTile: icon + label, then the value */
const tile=(icon,labelText,value,cls='')=>'<div class="sess-tile'+(cls?' '+cls:'')+'"><span>'+icon+esc(labelText)+'</span>'+value+'</div>';
function agentCard(row){
  const vendor=vendorOf(row),state=collectionState(row),usage=agentUsage(row.id);
  const busy=srcBusy.has(row.id);
  const draft=srcDraft.has(row.id)?srcDraft.get(row.id):row.path.configured;
  const dirty=draft!==row.path.configured;
  const note=srcNote.get(row.id);
  const costKnown=row.cost_status!=='unavailable';
  const inputId='sess-src-path-'+row.id;
  const tab=srcTab.get(row.id)||(state.key==='down'||dirty?'data':'usage');
  const pathState=!row.path.editable?'':'<span class="sess-path-state" data-tone="'+(row.path.exists?'ok':'warn')+'"><i></i>'+(row.path.exists?'Found':'Not found')+'</span>';
  const overview='<div class="sess-tiles">'
      +tile(icons.sparkles,'Sessions','<b>'+usage.sessions.toLocaleString()+'</b>')
      +tile(icons.layers,'Tokens','<b>'+tok(usage.tokens)+'</b>')
      +tile(icons.coins,'Cost','<b>'+(costKnown?usd(usage.cost):'—')+'</b>')
      +tile(icons.activity,'Last 30 days','<div class="sess-tile-spark" id="sess-spark-'+esc(row.id)+'"></div>','sess-tile-wide')
    +'</div>';
  const details=(row.path.editable?field({
      htmlFor:inputId,
      label:esc(row.path.label)+pathState,
      control:'<div data-slot="input-group">'+input({id:inputId,value:draft,placeholder:row.path.default,mono:true,disabled:busy,attrs:'data-path-input="'+esc(row.id)+'" aria-label="'+esc(row.path.label)+'"'})
        +button(busy?spinner('Saving'):'Save',{size:'sm',variant:dirty?'default':'outline',disabled:!dirty||busy,attrs:'data-path-save="'+esc(row.id)+'"'})
        +(row.path.configured?button('Reset',{variant:'ghost',size:'sm',disabled:busy,attrs:'data-path-reset="'+esc(row.id)+'" title="Back to '+esc(row.path.default)+'"'}):'')
        +'</div>',
      error:note?.tone==='error'?esc(note.text):state.key==='down'?esc(state.detail):'',
    }):'')
    +(note&&note.tone==='ok'?'<div class="sess-agent-note">'+icons.check+esc(note.text)+'</div>':'')
    +'<ul class="sess-features">'+row.collects.map(line=>'<li>'+icons.check+esc(line)+'</li>').join('')
      +(row.never?'<li class="is-muted">'+icons.shield+esc(row.never)+'</li>':'')+'</ul>';
  return card({cls:'sess-agent-card',attrs:'data-vendor="'+esc(row.vendor)+'" data-source="'+esc(row.id)+'" data-collection="'+state.key+'" style="--vendor:'+vendor.color+'"',
    before:'<div class="sess-agent-hero"><span class="sess-agent-mark">'+vendorMark(row)+'</span>'
      +'<span class="sess-status-pill" data-state="'+state.key+'"><i></i>'+esc(state.label)+'</span></div>',
    title:esc(row.label),
    action:badge(esc(vendor.label),{variant:'outline',attrs:'data-vendor-tag'}),
    description:'Powered by '+esc(vendor.label)+(usage.last?' · active '+esc(relTime(usage.last)):''),
    content:tabs({value:tab,listCls:'w-full',attrs:'data-card-tabs="'+esc(row.id)+'"',items:[
      {value:'usage',label:'Overview',content:overview},
      {value:'data',label:'Details',content:details},
    ]}),
    footer:label(switchControl({checked:row.enabled,disabled:busy,attrs:'data-collect="'+esc(row.id)+'" aria-busy="'+busy+'"'})+'<span>Collect sessions</span>',{attrs:'class="sess-agent-switch"'})
      +'<code class="sess-agent-id">@'+esc(row.id)+'</code>',
    footerCls:'border-t',
  });
}
function rConfigure(pageHead){
  ...
  let body='';
  if(srcLoading&&SRC===null)body='<div class="sess-agent-grid" role="status" aria-label="Loading sources">'+[0,1,2].map(()=>card({before:skeleton('height:120px;border-radius:12px 12px 0 0'),content:skeleton('height:20px;width:50%')+skeleton('height:90px;margin-top:16px')})).join('')+'</div>';
  ...
  for(const row of SRC){
    const spark=document.getElementById('sess-spark-'+row.id);
    if(spark)sparkline(spark,{data:agentUsage(row.id).days,config:{v:{label:'Tokens',color:'var(--vendor)'}},format:tok,xFormat:d=>d.slice(5),height:40});
  }
  wrap.querySelectorAll('[data-card-tabs]').forEach(root=>wireTabs(root,value=>{
    srcTab.set(root.dataset.cardTabs,value);
    /* a sparkline drawn while its panel was hidden has no width; redraw */
    const row=SRC.find(r=>r.id===root.dataset.cardTabs),spark=root.querySelector('.sess-tile-spark');
    if(value==='usage'&&row&&spark)sparkline(spark,{data:agentUsage(row.id).days,config:{v:{label:'Tokens',color:'var(--vendor)'}},format:tok,xFormat:d=>d.slice(5),height:40});
  }));
  wrap.querySelectorAll('[data-path-input]').forEach(box=>{
    box.addEventListener('input',()=>{
      const id=box.dataset.pathInput,row=SRC.find(r=>r.id===id);
      srcDraft.set(id,box.value);
      const save=wrap.querySelector('[data-path-save="'+CSS.escape(id)+'"]');
      if(save)save.disabled=box.value===row.path.configured||srcBusy.has(id);
    });
    box.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();wrap.querySelector('[data-path-save="'+CSS.escape(box.dataset.pathInput)+'"]')?.click();}});
  });
