...
// Add belongs to Manage; internal data invalidation retains mounted state.
...
async function reread(id,path,verify){
  ...
  const pending=hold(path);await startDataRefresh(page);await within(pending.arrived.promise,id+' refresh read');
  ...
  pending.release.resolve();await waitForDataRefresh(page);
...
async function layout(id,width){
  const geometry=await page.locator('#section-nav').evaluate(nav=>{
    ...
    const nodes=[document.querySelector('#root-btn')];
  ...
  assert.equal(geometry.nodes.length,1);assert.ok(geometry.scroll<=width);
...
try{
  ...
  checked('Internal data invalidation rereads the List catalog while preserving query, pins, drawer and current folder.');
  ...
  checked('Overview and Graph data invalidation replaces the active dataset without a document reload and retains the chosen root.');
  ...
  checked('Tree, Timeline and Sharing internal rereads display new source data; Tree search and Timeline mode/query persist.');
  ...
  checked('Manage owns Add; opening it leaves the form closed, while Add focuses the clone form and internal rereads retain its draft.');
  ...
  checked('All six Projects pages keep the root picker in secondary navigation and Manage as a page at 1440, 390 and 320px.');
