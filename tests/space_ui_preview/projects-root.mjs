...
const activeId=route=>route==='projects/files/list'?'projects':route==='projects/files/tree'?'tree':route==='projects/sharing'?'sharing':route==='projects/timeline'?'time':'graph';
...
let page,hold;
try{
  for(const route of ['projects/files/list','projects/files/tree','projects/sharing','projects/timeline']){
  ...
  checked('Direct Files List, Files Tree and Sharing do not boot the atlas before root selection; all three pages and Timeline open Files Graph once at the selected root and preserve history and control identity.');

  hold={arrived:gate(),release:gate()};
  page=await createPage(hold);
  await page.goto(origin+'/space/#/projects/sharing',{waitUntil:'domcontentloaded'});await active(page,'projects/sharing');
