...
const projectRoot=page.locator('#setup-projects'),removeButton=page.locator('#setup-project-delete');
...
try{
  ...
  assert.equal(await page.locator('#setup-project-id').inputValue(),'my-demo');
  cloneError='A folder with that name already exists.';await page.locator('#setup-project-create').click();
  await page.locator('#setup-project-add-error').waitFor();assert.equal(await page.locator('#setup-project-id').inputValue(),'my-demo');
  cloneError=null;const cloning=holdClone=gate();await page.locator('#setup-project-create').click();await cloning.arrived.promise;
  assert.equal(await page.locator('#setup-project-create').isDisabled(),true);
  ...
  assert.match(await page.locator('#setup-project-access').textContent(),/Charlie <local>/);
  ...
  for(const user of ['alice-space','bob-space']){
    ...
    await page.waitForFunction(()=>!document.querySelector('#setup-project-recheck').disabled);
  ...
  await page.waitForFunction(()=>!document.querySelector('#setup-project-recheck').disabled);
  assert.match(await page.locator('#setup-project-access').textContent(),/Access checked/);
  assert.match(await page.locator('#setup-project-access').textContent(),/Revoked/);
  assert.equal(await removeButton.isDisabled(),true,'Typing the project id is still required');
  await page.locator('#setup-project-confirm').fill('shared-demo-wrong');assert.equal(await removeButton.isDisabled(),true);
  await page.locator('#setup-project-confirm').fill('shared-demo');assert.equal(await removeButton.isEnabled(),true);
  ...
  assert.equal(await removeButton.isDisabled(),true);assert.equal(await page.locator('#setup-project-close').isDisabled(),true);
  ...
  assert.match(await page.locator('#setup-project-access').textContent(),/Only the project owner/);
  ...
  await openRemove('unknown-demo');await page.locator('#setup-project-confirm').fill('unknown-demo');
  assert.equal(await removeButton.isDisabled(),true);
  faults.set('unknown-demo',{data:{project_id:'unknown-demo',can_remove:true,blockers:[]}});
  await page.locator('#setup-project-recheck').click();await page.waitForFunction(()=>!document.querySelector('#setup-project-recheck').disabled);
  assert.equal(await removeButton.isDisabled(),true);assert.match(await page.locator('#setup-project-access').textContent(),/could not be verified/);
  ...
  await page.waitForFunction(()=>document.querySelector('#setup-project-access')?.textContent.includes('Only the project owner'));
  assert.equal(await page.locator('#setup-project-removal-id').textContent(),'incoming-demo');
  assert.equal(await removeButton.isDisabled(),true);
  await close();
  await openRemove('solo-demo');await page.locator('#setup-project-confirm').fill('solo-demo');
  const before=gate();holds.set('solo-demo',before);await page.locator('#setup-project-recheck').click();await before.arrived.promise;
  assert.equal(await removeButton.isDisabled(),true);
  members.set('solo-demo',[{...owner},{workspace_id:'arrived-during-check',role:'member',status:'active',can_revoke:true,is_self:false}]);
  const after=gate();holds.set('solo-demo',after);await page.locator('#setup-project-refresh').click();
  ...
  assert.equal(await removeButton.isDisabled(),true);
  assert.equal(await page.locator('#setup-project-confirm').inputValue(),'solo-demo','Refresh keeps the typed confirmation but rechecks its authority');
  ...
  assert.match(await page.locator('#setup-project-remove-error').textContent(),/Access changed/);
  ...
  await page.locator('#setup-project-add').click();
  await page.locator('#setup-project-repository').fill('https://github.com/fixture/keep-this-draft.git');
