...
class QuirqCatalogTests(unittest.TestCase):
    def test_catalog_summarizes_state_without_exposing_secret_or_session_ids(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / "jobs" / "runs").mkdir(parents=True)
            (root / "jobs" / "jobs.json").write_text('{"secret":"do-not-return"}')
            (root / "jobs" / "state.json").write_text('{"output_tail":"do-not-return"}')
            (root / "jobs" / "runs" / "demo.jsonl").write_text('{"output_tail":"do-not-return"}\n')
            ...
            self.assertIn("Saved manual commands", tree["jobs"]["description"])
            self.assertIn("Command execution", tree["jobs/state.json"]["description"])
            self.assertIn("output tail", tree["jobs/runs/demo.jsonl"]["description"])
