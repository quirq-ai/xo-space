...
class ReaderTests(_Sample):
    ...
    def test_job_runs_and_posts(self) -> None:
        ...
        self.assertEqual([p["key"] for p in posts], ["post:c0ffee02", "post:c0ffee01"], "history.json posts first, then the retired Inbox's")
...
class AttentionTests(_Sample):
    ...
    def test_blocked_todos_my_issues_connections_questions_and_errors(self) -> None:
        ...
        # the sample's gmail folder turns unread mail into items, which replace the raw decision; unlist it here
        with patch.object(attention, "_item_collectors", return_value={}):
        ...
        self.assertEqual(attention.connection_items(doc, {}), [], "listed as an item collector, the raw event is not a second decision")
        questions = attention.question_items(doc, {}, {})
        self.assertEqual([q["key"] for q in questions], ["post:c0ffee02", "post:c0ffee01"])
    ...
    def test_derive_orders_by_reason_and_never_fails_on_one_source(self) -> None:
        ...
        self.assertEqual(reasons, ["unassigned", "todo_blocked", "agent_question", "agent_question"])
